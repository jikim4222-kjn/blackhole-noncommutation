...
Let \(S_O\) be a coarse‑graining on subsystem \(A\) (binning, classicalization, etc.).
...
### Non‑invertibility
If subsystem \(B\) carries coherence that affects \(S(\rho)\) but is erased by \(\Pi\), then:
- \(S\) preserves distinctions  
- \(\Pi\) erases them  

Thus \(\eta_X\) cannot be invertible.  
This provides a concrete, operational instance of persistent non‑commutation.
